...
const getNames = (arr) => {
  let revArray = [];
  arr.map((rev, index) => {
    let nameChange = rev.name;
    let reverse = nameChange.split('').reverse().join('');
    return revArray.push(reverse);
  });
  return revArray;
};
...
const appendTheEnd = (str) => {
  let newEnd = 'The end.';
  let realCopyOfStory = str.concat(' ', newEnd);
  return realCopyOfStory;
};
