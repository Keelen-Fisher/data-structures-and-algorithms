...
const screenForNames = (arr) => {
  let proper = /^(Mr\.|Mrs\.|Ms\.|Dr\.)\s[a-zA-Z]+$/;
  return arr.filter( str => proper.test(str));
};
...
const toTitleCase = (arr) => {
  return arr.map(value=> value[0].toUpperCase() + value.slice(1));
};
...
let biggerThanLuke = (arr) => {
  ...
  let maceWindu = parseInt(arr.find(value => value.name === 'Luke Skywalker').mass);
  return arr.filter(value => value.mass > maceWindu).map(val => val.name).join(' - ');
};
...
const sortBy = (property, arr) => {
  return arr.sort((value, index) => value[property] < index[property] ? -1 : 1);
};
...
const detectTicTacToeWin = (board) => {
  ...
  let columBoard = [0,1,2].map((value) => [newBoard[0+value],newBoard[3+value],newBoard[6+value]]);
  let crossBoard = [[newBoard[0],newBoard[4],newBoard[8]],[newBoard[2],newBoard[4],newBoard[6]]];
  let lastCheck = [...crossBoard,...columBoard,...board];
  return lastCheck.findIndex(val => val.every(point => point === 'X') || val.every(point => point === 'O')) !== -1;
};
