...
const toLastNames = people => {
  return people.map(function(peoples, index){
    let newList = {};
    newList = peoples.firstName + ' ' + peoples.lastName;
    return newList;
  });
};
...
const addValues = (arr) => {
  let sum = arr.reduce(function(addedValue, value, index){
...
const addPurchases = (arr) => {
  let numberOnly = arr.reduce(function(num, value, index){
    return num + value.purchasePrice;
  },0);
  return numberOnly;
};
...
const returnNames = (arr) => {
  let nameOnly = arr.reduce(function(retNames, value, index){
    retNames.push(value.name);
    return retNames;
  },[]);
  return nameOnly;
};
